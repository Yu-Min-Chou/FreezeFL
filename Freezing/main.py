...
def main():
    ...
    for key in server.sys_metrics:
        server.fw.write("%f\n" % (server.sys_metrics[key]))

    clients = sorted(clients, key=lambda x: x.task_time(profile_result, args.num_local_epochs), reverse=True)
    participate_sum = [0] * SYSTEM_PARAMS['CAP_TIER']
    for i, c in enumerate(clients):
        participate_sum[math.floor(i/len(clients)*SYSTEM_PARAMS['CAP_TIER'])] += c.participate_time
...
def create_clients(users, train_data, test_data, model):
    ...
    default_utitlity = 10
    clients = [Client(u, train_data[u], test_data[u], model, capability[i], deafult_selected_probability, default_utitlity) for i, u in enumerate(users)]
